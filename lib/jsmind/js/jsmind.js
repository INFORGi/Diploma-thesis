...
export class jsMind {
    ...
    async createNodeElement(data) {
        ...
        // Рендерим markdown и ждём завершения
        ...
        // Настраиваем размеры в зависимости от фигуры
        if (data.figure.tag === 'path' && data.figure.dNormalized) {
            const edges = this.findEdges(data.figure.dNormalized);
            const topEdgeLength = edges.top ? this.getEdgeLength(edges.top) : 1;
            const bottomEdgeLength = edges.bottom ? this.getEdgeLength(edges.bottom) : 1;
            const leftEdgeLength = edges.left ? this.getEdgeLength(edges.left) : 1;
            const rightEdgeLength = edges.right ? this.getEdgeLength(edges.right) : 1;
    
            const horizontalRatio = Math.min(topEdgeLength, bottomEdgeLength);
            const verticalRatio = Math.min(leftEdgeLength, rightEdgeLength);
    
            const finalWidth = Math.max(contentRect.width / horizontalRatio + padding * 2, 150);
            const finalHeight = Math.max(contentRect.height / verticalRatio + padding * 2, 50);
    
            node.style.width = `${finalWidth}px`;
            node.style.height = `${finalHeight}px`;
    
            const safetyMargin = 0.9;
            topic.style.maxWidth = `${horizontalRatio * safetyMargin * 100}%`;
            topic.style.maxHeight = `${verticalRatio * safetyMargin * 100}%`;
        } else {
            node.style.width = `${Math.max(contentRect.width + padding * 2, 150)}px`;
            node.style.height = `${Math.max(contentRect.height + padding * 2, 50)}px`;
            topic.style.maxWidth = '90%';
            topic.style.maxHeight = '90%';
        }
        ...
        const drawFigure = () => {
            ...
            if (data.figure.dNormalized) {
                this.drawPath(ctx, data.figure.dNormalized, canvas.width, canvas.height, data.figure.rx);
            }
            ctx.fill();
            ctx.stroke();
        };
        ...
        drawFigure();
        const resizeObserver = new ResizeObserver(drawFigure);
    ...
    addChild(parentId) {
        ...
        const newNodeData = {
            ...
            topic: { ...this.deepCloneStyle(DEFAULT_NODE_DATA.topic), 
                        color: parentNode.data.topic.color, 
                        fontSize: parentNode.data.topic.fontSize,
                        fontFamely: parentNode.data.topic.fontFamely},
            parent: parentId,
            children: [],
    ...
    setupTopicEventListeners(topic) {
        ...
        topic.addEventListener('blur', async () => {
            ...
            if (nodeData.data.figure.tag === 'path' && nodeData.data.figure.dNormalized) {
                const edges = this.findEdges(nodeData.data.figure.dNormalized);
                ...
                node.style.width = `${Math.max(contentRect.width / horizontalRatio + padding * 2, 150)}px`;
                node.style.height = `${Math.max(contentRect.height / verticalRatio + padding * 2, 50)}px`;
            } else {
                node.style.width = `${Math.max(contentRect.width + padding * 2, 150)}px`;
                node.style.height = `${Math.max(contentRect.height + padding * 2, 50)}px`;
            }
    ...
    swapTheme() {
        if (!this.options.theme) { console.error("Не определена тема в опциях"); return; }
        ...
        if (!this.nodes) { console.error("nodes оказался пуст"); return; }

        this.nodes.forEach((id, data) => {
            if (!data.data.figure.fill || !data.data.figure.stroke || !data.data.topic.color) {
    ...
    getEdgeLength(points) {
        ...
        // Для фигур с fixedOffset используем другой расчет
        if (p1.fixedOffset !== undefined || p2.fixedOffset !== undefined) {
            // Если точки имеют fixedOffset, это влияет на реальную длину ребра
            const p1Offset = p1.fixedOffset !== undefined ? p1.fixedOffset : 0;
            const p2Offset = p2.fixedOffset !== undefined ? p2.fixedOffset : 0;
            
            // Рассчитываем максимальное смещение от краев
            const totalOffset = p1Offset + p2Offset;
            
            // Возвращаем соотношение доступного пространства
            return 1 - (totalOffset / 150); // 150 - минимальная ширина узла
        }

        // Для обычных точек используем стандартный расчет
