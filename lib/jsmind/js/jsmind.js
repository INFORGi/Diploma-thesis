...
export class jsMind {
    ...
    async createNodeElement(data) {
        ...
        const { width, height, maxWidth, maxHeight } = this.calculateNodeDimensions(data.figure, contentRect);
        ...
        this.drawNodeFigure(canvas, containerWithTextShape, data.figure);
    
        const resizeObserver = new ResizeObserver(() => this.drawNodeFigure(canvas, containerWithTextShape, data.figure));
    ...
    addChild(parentId) {
        ...
        const newNodeData = {
            ...
            parent: parentId,
            children: [],
            figure: this.deepCloneStyle(FIGURE.RECTANGLE),
            styleNode: this.deepCloneStyle(NODE_STYLES),
            styleTopic: this.deepCloneStyle(TOPIC_STYLES),
            styleLine: this.deepCloneStyle(LINE_STYLES.BEZIER),
            position: { x: 0, y: 0 },
            draggable: true
        };
...
　　　　　　　topic.focus();
        ...
        topic.addEventListener('blur', async () => {
            ...
            const contentRect = topic.getBoundingClientRect();
            const { width, height, maxWidth, maxHeight } = this.calculateNodeDimensions(nodeData.data.figure, contentRect);
    ...
    calculateNodeDimensions(figure, topicRect) {
        ...
        if (figure.tag === 'path' && figure.dNormalized) {
            ...
            maxWidth = `${horizontalRatio * 0.9 * 100}%`;
            maxHeight = `${verticalRatio * 0.9 * 100}%`;
        } else {
    ...
    drawNodeFigure(canvas, container, figure) {
        ...
        if (figure.dNormalized) {
            this.drawPath(ctx, figure.dNormalized, canvas.width, canvas.height, figure.rx);
        }
