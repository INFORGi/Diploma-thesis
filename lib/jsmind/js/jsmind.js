...
export class jsMind {
    ...
    createNode(data, parent) {
        ...
        function getContrastColor(color) {
            const rgb = window.getComputedStyle(color).color.match(/\d+/g);
            if (!rgb) return 'white';
        
            const brightness = (0.299 * rgb[0] + 0.587 * rgb[1] + 0.114 * rgb[2]) / 255;
        
            return brightness > 0.5 ? 'rgba(0, 0, 0, 0.1)' : 'rgba(255, 255, 255, 0.8)'; 
        }
        ...
        topic.addEventListener('dblclick', (e) => {
            ...
            topic.style.backgroundColor = getContrastColor(topic);
        ...
        // Улучшаем наследование стилей от родителя
        if (parent) {
            const parentNode = this.nodes.get(parent);
            if (parentNode && parentNode.element.nodeData) {
                // Наследуем стили узла от родителя, если у текущего узла их нет
                if (!data.style) {
                    data.style = { ...parentNode.element.nodeData.nodeStyle };
                } else {
                    // Объединяем стили, сохраняя приоритет стилей текущего узла
                    data.style = {
                        ...parentNode.element.nodeData.nodeStyle,
                        ...data.style
                    };
                }

                // Наследуем стили текста от родителя
                if (!data.topicStyle) {
                    data.topicStyle = { ...parentNode.element.nodeData.topicStyle };
                } else {
                    // Объединяем стили текста, сохраняя приоритет стилей текущего узла
                    data.topicStyle = {
                        ...parentNode.element.nodeData.topicStyle,
                        ...data.topicStyle
                    };
                }
            }
        ...
        // Сохраняем данные узла с обновленными стилями
        node.nodeData = {
        ...
        // Применяем стили текста к topic
        if (data.topicStyle) {
            Object.assign(topic.style, data.topicStyle);
            // Применяем выравнивание текста
            topic.style.textAlign = data.topicStyle.textAlign || 'center';
            node.style.display = 'flex';
            node.style.alignItems = data.topicStyle.verticalAlign || 'center';
            node.style.justifyContent = data.topicStyle.textAlign === 'left' ? 'flex-start' : 
                                      data.topicStyle.textAlign === 'right' ? 'flex-end' : 
                                      'center';
        }
    ...
    add_node(parentId, topic = 'Текст') {
        ...
        const newNodeData = {
            ...
            // Наследуем стили от родительского узла
            style: parentElement.nodeData ? { ...parentElement.nodeData.nodeStyle } : { ...DEFAULT_NODE_STYLE },
            topicStyle: parentElement.nodeData ? { ...parentElement.nodeData.topicStyle } : {}
        };
    ...
    getNodeData(nodeId) {
        ...
        return {
            ...
            topicStyle: topic ? {
                ...
                textAlign: topic.style.textAlign || 'center',
                verticalAlign: element.style.alignItems || 'center'
            } : {},
