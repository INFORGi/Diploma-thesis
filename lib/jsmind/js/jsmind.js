...
export class jsMind {
    ...
    createNode(data) {
        const nodeData = {
            ...
            styleNode: data.styleNode || { ...NODE_STYLES.RECTANGLE },
            styleTopic: data.styleTopic || { ...TOPIC_STYLES },
            styleLine: data.styleLine || { ...LINE_STYLES.DEFAULT },
    ...
    createNodeElement(data) {
        ...
        if (data.styleNode.shapeSvg) {
            ...
            const polygon = document.createElementNS("http://www.w3.org/2000/svg", data.styleNode.shapeSvg.tag);
            polygon.setAttribute('points', data.styleNode.shapeSvg.points);
            polygon.setAttribute('fill', data.styleNode.shapeSvg.fill);
            polygon.setAttribute('stroke', data.styleNode.shapeSvg.stroke);
            polygon.setAttribute('stroke-width', data.styleNode.shapeSvg.strokeWidth);

            shape.appendChild(polygon);
            node.appendChild(shape);
        } else {
            Object.assign(node.style, data.styleNode.styles);
        }
    ...
    makeNodeDraggable(node) {
        ...
        const handleMouseMove = (e) => {
            ...
            // Обновляем данные о позиции в Map
        ...
        const handleMouseUp = () => {
            ...
            // Проверяем, есть ли узел поблизости для присоединения
            ...
            if (nearestNode && nearestNode.element.id !== node.id) {
                ...
                // После смены родителя обновляем только позиции дочерних элементов

                this.layoutChildren(nearestNode.element.id, nearestNode.element.width);
            }

            // Перерисовываем линии без полного layout
            this.drawLines();
        };
    ...
    findNearestNode(draggingNode) {
        ...
        const threshold = Math.min(draggingRect.width, draggingRect.height) / 2; // 20 в вашем случае
        let nearest = null;
        let minDistance = Infinity;

        this.nodes.forEach((node) => {
            // if (node.element === draggingNode || node.element.dataset.isroot === 'true') return;
    ...
    addChild(parentId) {
        const newNodeData = {
            ...
            styleLine: { ...LINE_STYLES.BEZIER },
            position: { x: 0, y: 0 },
            draggable: true
        };

        console.log('New node styles:', newNodeData.styleNode);
