...
export class jsMind {
    ...
    createNode(data, parent) {
        ...
        if (node.nodeData.position) {
            node.style.left = `${node.nodeData.position.x}px`;
            node.style.top = `${node.nodeData.position.y}px`;
        }
    ...
    layoutNewChildren(nodeId) {
        ...
        if (newChildren.length > 0) {
            ...
            newChildren.forEach((childId, index) => {
                ...
                const y = parentY + (index * vspace) - ((newChildren.length - 1) * vspace / 2);
    ...
    getNodeData(nodeId) {
        ...
        const nodeData = {
            ...
            type: node.data.type,
            style: node.element.nodeData.nodeStyle || {},
            children: []
        };

        // Рекурсивно собираем данные дочерних узлов
        node.children.forEach(childId => {
            const childData = this.getNodeData(childId);
            if (childData) {
