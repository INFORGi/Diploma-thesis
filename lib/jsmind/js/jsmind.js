...
export class jsMind {
    ...
    async createNode(data) {
        ...
        this.getPosition(nodeData.id); // Позиционируем узел
    ...
    async layout(nodeId = this.root) {
        ...
        // Позиционируем только корневой узел
        if (nodeId === this.root) {
            ...
            // Позиционируем дочерние узлы корня
            node.children.forEach(childId => this.getPosition(childId));
        }
    ...
    addChild(parentId) {
        ...
        const newNodeData = {
            ...
            position: { x: 0, y: 0 }, // Начальные значения, будут обновлены в getPosition
            draggable: true
        };
