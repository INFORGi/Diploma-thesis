    ...
    removeNode(nodeId) {
        ...
        if (this.options.cascadeRemove) {
            ...
            if (parentNode) {
                ...
                // Перемещаем всех детей удаляемого узла к его родителю
                node.children.forEach(childId => {
                    const childNode = this.nodes.get(childId);
                    if (childNode) {
                        childNode.parent = parentNode.data.id;
                        childNode.data.parent = parentNode.data.id;
            ...
            node.element.remove();
            this.nodes.delete(nodeId);
            if (this.activeNode === node.element) {
                this.activeNode = null;
            }
        }
