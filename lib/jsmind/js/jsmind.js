import { FIGURE, TOPIC_STYLES, NODE_STYLES, LINE_STYLES, DEFAULT_NODE_DATA, MIND_MAP_THEMES } from '../../../data/constants.js';
...
export class jsMind {
    ...
    async layout() {
        ...
        const elementRect = rootNode.element.getBoundingClientRect(); // Теперь это DOM-элемент
    ...
    setActiveNode(node) {
        // Сбрасываем выделение с предыдущего узла
        if (this.activeNode && this.activeNode !== node) {
            this.activeNode.style.zIndex = '1';
            this.activeNode.style.border = 'none'; // Убираем обводку с предыдущего узла
        }
        if (node) {
            node.style.zIndex = '1000';
            node.style.border = MIND_MAP_THEMES[this.options.theme].selectNode || '2px solid #00f'; // Устанавливаем обводку
            this.activeNode = node;
        } else {
            // Если node === null, сбрасываем активный узел
            if (this.activeNode) {
                this.activeNode.style.zIndex = '1';
