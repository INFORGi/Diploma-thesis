import { NODE_STYLES, TOPIC_STYLES } from '../../../data/constants.js';
...
export class NodeStyleManager {
    ...
    changeNodeShape(node, shapeName) {
        const nodeData = this.jm.nodes.get(node.id);
        if (!nodeData || !NODE_STYLES[shapeName]) return;
        ...
        nodeData.data.styleNode = JSON.parse(JSON.stringify(NODE_STYLES[shapeName]));
